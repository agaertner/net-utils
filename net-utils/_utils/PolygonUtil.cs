...
namespace agaertner.NetUtils
{
    public static class PolygonUtil
    {
        ...
        public static bool TryGet(List<Vector2> points, out List<Vector2> result)
        {
            result = points;
            if (points == null)
                return false;
        ...
        public static bool Triangulate(Vector2[] vertices, out int[] triangles)
        {
            ...
            while (indexList.Count > 3)
            {
                ...
                {
                    var a = indexList[i];
                    int b = i - 1 >= 0 ? indexList[i - 1] : 0;
                    int c = i + 1 <= indexList.Count - 1 ? indexList[i + 1] : 0;
